...
//		ConfigurableApplicationContext 인터페이스 타입으로 환경설정 정보를 읽어올 객체( DI 컨테이너)를 만든다.
		ConfigurableApplicationContext ctx = new GenericXmlApplicationContext();
		
//		ConfigurableApplicationContext 인터페이스 타입의 객체에서 DI 컨테이너 환경설정 정보를 getEnvironment() 메소드로 얻어와서
...
//		getPropertySources() 메소드로 properties 정보만 얻어와서 저장한다.
		...
		try {
//		DI 컨테이너에서 얻어온 properties 정보가 저장되는 MutablePropertySources 클래스 객체 mutablePropertySources에 addLast() 메소드를
//		사용해서 맨 마지막 위치에 admin.properties 파일의 properties 내용을 읽어 추가한다.
			mutablePropertySources.addLast(new ResourcePropertySource("classpath:/admin.properties"));
			
//			DI 컨테이너 환경에 추가된 properties 파일의 내용은 ConfigurableEnvironment 인터페이스 객체에서 getProperty() 메소드로 읽어올 수 있다.
...
//		AdminConnection 클래스의 bean이 생성된 후 환경설정 정보가 저장된 admin.properties 파일의 정보를 넘겨준다.
		System.out.println("AdminConnection 클래스의 bean이 생성도니 후 환경 설정 정보에 저장된 properties 파일의 정보를 넣어준다." );
